                ...
                with open(names_file_data[i], 'r') as f:
                    ...
                    for j in range(10):
                        ...
                        if isArticleInThisYear(date, year):
                            ...
                            print("\n\n\t***\tTitle is:\t***")
                            print(title)
                            print("\t***\tEntities for title are:\t***")
                            for ent in doc.ents:
                                print(ent.text, ent.start_char, ent.end_char, ent.label_)
                            doc = nlp(body)
                            print("\t***\tEntities for data are:\t***")
                            for ent in doc.ents:
                                print(ent.text, ent.start_char, ent.end_char, ent.label_)
        ...
        if (ent.label_ == "TIME"):
            ...
            for ent2 in doc.ents:
                for label in important_labels:
                    if (ent2.label_ == label):
                        print(ent2.text, ent2.start_char, ent2.end_char, ent2.label_)
                        break
            nb_displayed += 1
            if nb_displayed == 10:
                exit(0)
            break
    handle_article_rake_nltk(body, 2)


def handle_article_2(title, body):
    global nb_displayed
    doc = nlp(title)
    for ent in doc.ents:
        if (ent.label_ == "TIME"):
            ...
            for ent2 in doc.ents:
                for label in important_labels:
                    if (ent2.label_ == label):
                        print(ent2.text, ent2.start_char, ent2.end_char, ent2.label_)
                        sentences = [sentence for sentence in body.split('.') if ent2.text in sentence]
                        for sentence in sentences:
                            print(sentence)
            nb_displayed += 1

            handle_article_rake_nltk(body, 4)
            if nb_displayed == 10:
                exit(0)
                break


def handle_article_3(title, body):
    global nb_displayed
    doc = nlp(body)
    dates = [] # composed of the date and the weight # 1 per apparitions, 0.5 if before 100 characters
    for ent in doc.ents:
        ...
        if (ent.label_ == "TIME"):
            for ent2 in doc.ents:
                if (ent2.label_ == "TIME" or ent2.label_ == "DATE"):
                    ...
                    # todo:
                    value = 1
                    # bonus of 0.5 if was at begining of the text or if is a time
